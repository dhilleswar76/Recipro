...
For a minimal Vercel deployment, configure only the required variables below. Vercel sets `NODE_ENV` automatically.
...
| Variables | Used for |
| :--- | :--- |
| `GEMINI_API_KEY` | Live AI generation; fallback is used when absent |
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `EMAIL_FROM` | SMTP email delivery |
