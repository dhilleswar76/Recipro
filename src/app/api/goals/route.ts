...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const skillResult = await query('SELECT id FROM skills WHERE LOWER(name) = LOWER($1)', [skillName]);
    ...
    `, [
      `goal-${user.userId}-${skill.id}`, user.userId, skill.id, targetProficiency,
      priority, notes || '', learningDays, availableStartTime, availableEndTime,
      preferredStartTime, preferredEndTime, sessionDurationMinutes, timezone, isFlexible,
    ]);
